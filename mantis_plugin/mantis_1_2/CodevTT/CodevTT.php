...
class CodevTTPlugin extends MantisPlugin {
  ...
  function filter_columns() {
   ...
   public function update_bug($event, BugData $p_bug_data) {
      ...
      if (!$this->isAccessGranted($p_bug_data->project_id)) {
         ...
         if (gpc_isset( 'codevtt_command_id' )) {
            ...
            // TODO le check gpc_isset marche pas dans le cas ou le user veut supprimer toutes les commandes:
            // la variable ne sera pas possitionnee, et les commandes non retirees...
            ...
            foreach ($prev_command_ids as $prev_cmd_id) {
              if (!in_array($prev_cmd_id, $new_command_ids)) {
                ...
                db_query_bound($delete_query, array( $p_bug_data->id, $prev_cmd_id ));
                ...
                db_query_bound($delete_from_wbs_query, array( $prev_cmd_id, $p_bug_data->id ));
   ...
   public function assignCommand($p_bug_id, $command_id) {
     ...
     db_query_bound($query, array( $command_id, $p_bug_id ) );
     ...
     if (is_null($wbs_id)) {
        ...
        db_query_bound($query3, array( 1, 1, $cmd_name ));
        $wbs_id = db_insert_id();

        $query4 = "UPDATE codev_command_table SET wbs_id = " . db_param() . " WHERE id = " . db_param();
        db_query_bound($query4, array( $wbs_id, $command_id ));
        ...
        db_query_bound($query5, array( $wbs_id, $wbs_id, $p_bug_id, $order, 0 ));
     }
   }
